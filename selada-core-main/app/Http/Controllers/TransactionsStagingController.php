 ...
 */
class TransactionsStagingController extends Controller
{
    /**
    ...
    public function store(TransactionCreateRequest $request)
    {
        ...
        try {
            ...
            if($service){
                ...
                $updateTrans->note = is_array($trx) ? $trx['data'] : null; //hanya untuk kebutuhan dev local saja
                $updateTrans->save();
	...
	if ($url != 'NULL'){
		$responseCurl = Curl::to($url)
                	->withData($reqData)
               		->asJson()
                	->post();
		if (isset($responseCurl) && !is_null($responseCurl)){
			if (isset($responseCurl->screen) && isset($responseCurl->screen->title)){
				$statusString = $responseCurl->screen->title;
...
//         return response()->json($response, 404);
    ...
    public function updateStatus(Request $request)
    {
        ...
        try {
            ...
            if($transaction){
                Log::info('Log message', $request->all());
